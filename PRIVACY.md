...
- `webNavigation`: Required to detect when a page is replaced by typing a new URL in the address bar (off by default, user must enable manually)
...
- `webNavigation`：用于检测用户在地址栏输入新网址时被替换的页面（默认关闭，需用户手动开启）
